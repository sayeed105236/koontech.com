...
$(function() {
	$('ul.side-user-menu li span.show-child-arrow').on('click', function(){
...
$(document).ready(function() {
    ...
    $(".copy-desc").on('click', function(index, element) {
        // $('.copy-desciption-content')
        const el = document.createElement('textarea');
        el.value = $('.copy-desciption-content').html().replace( /(<([^>]+)>)|[\s]+/ig, '');
    ...
    if($(window).width() > 992){
        let navtop = $( ".home-banner-area.nav-middle" ).offset().top;
        $( window ).scroll(function() {
            let top = $( window ).scrollTop(); 
            if(navtop < top){
                $( ".home-banner-area.nav-middle" ).addClass('stricky-nav');
            }else{
                $( ".home-banner-area.nav-middle" ).removeClass('stricky-nav');
            }
        }); 
    }
});
